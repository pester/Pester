...
Describe 'PesterThrowFailureMessage' {
    It 'returns false if the actual message is not the same as the expected message' {
        ...
        $result | Should Match "^Expected: the expression to throw an exception with message {$expectedErrorMessage}, an exception was raised, message was {$unexpectedErrorMessage}`n    from $([RegEx]::Escape("$PSScriptRoot\PesterThrow.Tests.ps1")):\d+ char:\d+"
    }
...
Describe 'NotPesterThrowFailureMessage' {
    It 'returns false if the actual message is not the same as the expected message' {
        ...
        $result | Should Match "^Expected: the expression not to throw an exception with message {$expectedErrorMessage}, an exception was raised, message was {$unexpectedErrorMessage}`n    from $([RegEx]::Escape("$PSScriptRoot\PesterThrow.Tests.ps1")):\d+ char:\d+"
    }

    It 'returns true if the actual message is the same as the expected message' {
        PesterThrow { throw 'error message' } > $null
        $result = NotPesterThrowFailureMessage 'error message'
        $result | Should Match "^Expected: the expression not to throw an exception. Message was {error message}`n    from $([RegEx]::Escape("$PSScriptRoot\PesterThrow.Tests.ps1")):\d+ char:\d+"
    }
}
