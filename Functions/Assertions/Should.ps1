...
function Should {
    ...
    end
    {
        ...
        {
            Invoke-Assertion $entry $parsedArgs $null $file $lineNumber $lineText
        }
        elseif ($entry.SupportsArrayInput)
        {
            Invoke-Assertion $entry $parsedArgs $inputArray.ToArray() $file $lineNumber $lineText
        }
        else
        {
            foreach ($object in $inputArray)
            {
                Invoke-Assertion $entry $parsedArgs $object $file $lineNumber $lineText
            }
        }
    }
}

function Invoke-Assertion($assertionEntry, $shouldArgs, $valueToTest, $file, $lineNumber, $lineText)
{
