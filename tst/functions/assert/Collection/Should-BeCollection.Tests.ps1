...
Describe "Should-BeCollection" {
    ...
    It "Fails when collections don't have the same count" -ForEach @(
        ...
        $err.Exception.Message | Verify-Equal "Expected [Object[]] @($($expected -join ", ")) to be equal to [Object[]] @($($actual -join ", ")), but they had a different count of items."
    }
    ...
    It "Fails when collections don't have the same items" -ForEach @(
        ...
        $err = { $actual | Should-BeCollection $expected } | Verify-AssertionFailed
        $err.Exception.Message | Verify-Equal "Expected [Object[]] @($($expected -join ", ")) to be equal to [Object[]] @($($actual -join ", ")), but they have different items."
    }
    ...
    It "Fails when collections don't have the same items" -ForEach @(
        ...
        $err = { $actual | Should-BeCollection $expected } | Verify-AssertionFailed
        $err.Exception.Message | Verify-Equal "Expected [Object[]] @($($expected -join ",")) to be equal to [Object[]] @($($actual -join ",")), but they don't have the same number of items."
    }
}
