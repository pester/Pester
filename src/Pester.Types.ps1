...
if ($null -ne $configurationType) {
    ...
    # we can load the Dll in tests to test more low-level constructs like Pester.Runtime separately
    # only check this if we are loading this as part of Pester module
    if ($null -ne $ExecutionContext.SessionState.Module -and "Pester" -eq $ExecutionContext.SessionState.Module.Name) {
