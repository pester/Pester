...
namespace Pester
{
    public class RunConfiguration : ConfigurationSection
    {
        ...
        public RunConfiguration() : base("General runtime options for Pester including tests containers to execute.")
        {
            ...
            BeforeContainer = new ScriptBlockArrayOption("EXPERIMENTAL: One or more ScriptBlocks run inside each parallel worker runspace before a test file is discovered and run. Use it to import helper modules or dot-source setup that the parent session would normally provide, e.g. { . './setup.ps1' }. One scriptblock is usually enough and can dot-source as many files as needed. When set, the auto-discovered convention file is ignored; when not set, Pester walks up from each test file to the repo root and dot-sources the first 'Pester.BeforeContainer.ps1' it finds. Only used when Run.Parallel is enabled.", new ScriptBlock[0]);
