function Should-NotContainCollection {
    ...
    if (Test-CollectionContains -Expected $Expected -Actual $Actual) {
        $Message = Get-AssertionMessage -Expected $Expected -Actual $Actual -Because $Because -DefaultMessage "Expected <expectedType> <expected> to not be present in collection <actual>,<because> but it was there."
