function Context {
    ...
    if ($ExecutionContext.SessionState.PSVariable.Get('invokedViaInvokePester')) {
        ...
        if ($PSBoundParameters.ContainsKey('ForEach')) {
            ...
            New-ParametrizedBlock -Name $Name -ScriptBlock $Fixture -StartLine $MyInvocation.ScriptLineNumber -StartColumn $MyInvocation.OffsetInLine -Tag $Tag -FrameworkData @{ CommandUsed = 'Context'; WrittenToScreen = $false } -Skip:$Skip -Reason:$Reason -Data $ForEach
        }
        else {
            New-Block -Name $Name -ScriptBlock $Fixture -StartLine $MyInvocation.ScriptLineNumber -Tag $Tag -FrameworkData @{ CommandUsed = 'Context'; WrittenToScreen = $false } -Skip:$Skip -Reason:$Reason
        }
    }
    else {
